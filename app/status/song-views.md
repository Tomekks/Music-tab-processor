...
**Not built yet:** a sequential/step-through overview mode for Fretboard, real rhythm notation on Sheet, the design-system tokens applied to Ascii's own chrome (Fretboard's are done; Ascii's `<pre>` block already used them from the start, see `AsciiView.tsx`).
