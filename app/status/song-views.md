...
Part of `app/status/` — see `app/STATUS.md` for the index. **This is current state only** — what's built, where the code lives, what's not built yet. The *why* behind each of these lives in `docs/decisions/0005-display-modes.md`; the exact rendering rules live in each component's own `RULES.md` — neither is repeated here.
