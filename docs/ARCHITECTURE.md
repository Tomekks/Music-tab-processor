...
- **Not yet built:** the local web UI for triggering pipeline runs (still file-path-based CLI commands), playback/metronome/visual note-highlighting, difficulty grading, `yt-dlp` ingestion. **A security review is now genuinely due** (see `docs/PENDING_ACTIONS.md`) — this is a real public URL now, not a local-only exercise.
