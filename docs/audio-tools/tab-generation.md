...
| [`open-fret`](https://github.com/Sidmaz666/open-fret) | backlog (documented swap-in "once it matures," see `docs/decisions/0004-backlog-and-scope.md`) | Custom T5 transformer (Fretting-Transformer architecture) trained specifically for ergonomic fingering — a learned approach rather than rule-based like the other three. New project (2025); maturity to be judged when actually tried. |
