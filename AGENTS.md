...
This repo's coding agent runs the [Superpowers](https://github.com/obra/superpowers) plugin (installed 2026-09-10, user-wide). Its skills — `brainstorming`, `writing-plans`, `test-driven-development`, `systematic-debugging`, `requesting-code-review`/`receiving-code-review`, `using-git-worktrees`, `finishing-a-development-branch`, `verification-before-completion` — are now the default source for *how* to plan, test, debug, review, and verify work, superseding this project's own earlier, looser versions of the same practices. See `docs/decisions/0009-agent-workflow-tooling.md` for the full reasoning on what changed below and why.
...
**Do the work directly in the primary session by default — don't delegate to subagents** (a Task/Agent tool, a forked/parallel worker session, etc.), including for fact-finding ("just reading a few files and summarizing" is exactly the inline case this rule covers, not an exception). A delegated call pays its own system-prompt and reasoning overhead on top of the work itself, and its output still lands back in the primary session anyway — for a codebase this size that's usually *more* total cost, not less, and this is a real-money hobby budget, not a free resource. Only delegate when there's a concrete reason: genuine parallelism across independent large tasks, or the primary session is actually at risk of running out of context — neither is the common case here.
...
- **When TDD's green step leaves more than one way to pass the test, use the decision ladder in `docs/decisions/0009-agent-workflow-tooling.md`** (adapted from the `ponytail` plugin, not installed) to pick the minimal implementation.
...
- **Every commit to `app/` runs through a pre-commit hook** (`.githooks/pre-commit`, a secret scan + typecheck) once `git config core.hooksPath .githooks` is run once for this clone. A block on a real credential means stop and rotate it, not `--no-verify` past it.
...
- `docs/plans/` — one folder per plan, plan doc + its own task specs together (standalone specs with no parent plan live flat in `docs/plans/specs/`); one spec file per task handed to an execution model: exact inputs/outputs, file locations, and a "done when" checklist. No ambiguity by design.
...
- **Ask before every git commit or push, not just `app/` changes, every single time.** Never infer permission from a broader instruction like "handoff" or "let's clean up" — a commit needs its own explicit yes, the same way a deletion does. (The `app/`-specific "Push to git? / Push to git & deploy? / Skip for now?" question below is the app case of this same general rule, not the whole of it.) **Exception:** a single task's own local checkpoint commit, mid-plan, is commit-and-report rather than ask-first — see `docs/WEB_APP_WORKFLOW.md` §4. Push, PR, deploy, and anything spanning more than the one task just completed still require an explicit ask.
